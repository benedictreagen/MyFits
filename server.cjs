    ...
    if (!ai) {
      return res.json({
        ...
        weeklyReview: {
          ...
          summary: "Excellent overall adherence this week. Your 7-day weight trajectory is right on pace without restrictive deficits."
        }
      });
    }
    const prompt = `
...
`;
    ...
    return res.status(200).json({
      ...
      weeklyReview: {
        ...
        summary: "Solid progress this week. Your habits are building lasting momentum."
      }
    });
  }
});
...
app.post("/api/ai/meal-recommendations", async (req, res) => {
  try {
    const {
      ...
      remainingBudget = {},
      mealType = "all",
      // 'breakfast' | 'lunch' | 'dinner' | 'snack' | 'all'
      recommendationType = "all",
      // 'all' | 'complete_meals' | 'individual_items'
      databaseFoods = []
    } = req.body;
...
- Daily Calorie Budget: ${nutritionTargets?.dailyCalorieBudget || 2e3} kcal
...
- Foods already logged: ${loggedFoods.length > 0 ? loggedFoods.map((f) => `${f.name} (${f.calories} kcal, P:${f.protein}g)`).join("; ") : "None yet"}
...
app.post("/api/ai/food-search", async (req, res) => {
  try {
    ...
    const ai = getGenAIClient();
    const dbSample = Array.isArray(databaseFoods) ? databaseFoods.slice(0, 30).map((f) => ({
      id: f.id,
      name: f.name,
      category: f.category,
      calories: f.calories,
...
`;
      ...
      if (parsed.similarDatabaseItems && Array.isArray(parsed.similarDatabaseItems)) {
        parsed.similarDatabaseItems = parsed.similarDatabaseItems.map((s) => {
    ...
    const similarDatabaseItems = dbSample.map((food) => {
      let score = 0;
    ...
    }).filter((item) => item.matchScore >= 40).sort((a, b) => b.matchScore - a.matchScore).slice(0, 3);
    return res.json({
      source: "rule-based",
      identifiedFood: {
        name,
...
async function startServer() {
  if (process.env.NODE_ENV !== "production") {
    ...
    app.use("/MyFits", import_express.default.static(distPath));
